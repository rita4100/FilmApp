...
async function openModal(id) {
  ...
  body.innerHTML = `
    ...
    <div class="film-modal-info">
      ...
      <details class="info-accordion soundtrack-section">
        <summary>Soundtrack</summary>
        <div id="soundtrack-list" class="accordion-content"><p class="ui-soft">Načítám soundtrack...</p></div>
      </details>
      ${film.cast && film.cast.length ? `<details class="info-accordion film-cast"><summary>Herecké obsazení</summary><div class="accordion-content">${film.cast.slice(0,10).map(p => `<div class="cast-item"><strong>${escapeHtml(p.person_name)}</strong>${p.character ? ` jako ${escapeHtml(p.character)}` : ''}</div>`).join('')}</div></details>` : '<details class="info-accordion film-cast"><summary>Herecké obsazení</summary><div class="accordion-content"><p class="ui-soft">Žádné herce jsme nenašli.</p></div></details>'}
      ${film.crew && film.crew.length ? `<details class="info-accordion film-crew"><summary>Klíčový tým</summary><div class="accordion-content">${film.crew.slice(0,8).map(p => `<div class="crew-item"><strong>${escapeHtml(p.person_name)}</strong>${p.job ? ` • ${escapeHtml(p.job)}` : ''}${p.department ? ` (${escapeHtml(p.department)})` : ''}</div>`).join('')}</div></details>` : ''}
      ${currentUser ? `
